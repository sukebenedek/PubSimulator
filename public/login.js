        ...
        if (userFinder(username, users) == null) {
            if (password.length > 0) {
                if (password.length > 20) {
                    ...
                    if (await postData("http://localhost:3000/users", { id: String(users.length), username: username, password: password, money: 1000, drunkness: 0, img: img, role: role, isServed: false, order: [] })) {
